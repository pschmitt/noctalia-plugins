...
- **My entities** — the configured list, each entity in its own card, showing its current state and domain-appropriate controls:
  ...
  - If **Panel: Show entity search** is enabled, the main view has a search bar that fuzzy-matches multi-term queries across names, entity IDs, sections, and displayed states (ranking actionable domains such as lights and switches ahead of sensors). Prefix a query with a domain and dot to restrict results, for example `light.marr` matches lights whose names or entity IDs contain “marr”, and `light.` lists lights. **Panel: Entity search position** places the search at the top or bottom (the bottom position stays visible while the entity list scrolls). Search results from Home Assistant are queried live in the background and merged with the local cache; section collapse state is preserved while searching. **Panel: Preserve state between openings** controls whether the main search term, section collapse state, and open entity details page survive closing and reopening the panel.

...
- **My entities** — the configured list, each entity in its own card, showing its current state and domain-appropriate controls:
  ...
  - If **Panel: Show entity search** is enabled, the main view has a search bar that fuzzy-matches multi-term queries across names, entity IDs, sections, and displayed states (ranking actionable domains such as lights and switches ahead of sensors). Prefix a query with a domain and dot to restrict results, for example `light.marr` matches lights whose names or entity IDs contain “marr”, and `light.` lists lights. A colored `FILTER: LIGHT` hint marks the active domain qualifier. Sections with matches expand while searching and return to their configured collapse state when the query is cleared. **Panel: Entity search position** places the search at the top or bottom (the bottom position stays visible while the entity list scrolls). Search results from Home Assistant are queried live in the background and merged with the local cache; **Panel: Preserve state between openings** controls whether the main search term, section collapse state, and open entity details page survive closing and reopening the panel.
  - The plugin builds a bounded background cache of Home Assistant's state-backed entity IDs and friendly names. Search results from that cache that are not in the layout appear below the configured results under **Other** (capped at 200 rows so the panel remains within Noctalia's UI child limit); opening one fetches its live state and shows the same domain-specific details and controls as a configured entity, without adding it to the layout. Its details view offers **Add to dashboard** when you decide to keep it.
  ...
  - **light** gets a state-aware bulb/bulb-off entity icon and a native stateful on/off switch, plus a brightness slider under the row whenever it's on and Home Assistant reports a brightness (i.e. it's dimmable).
  ...
  - Numeric **sensors** (as well as numbers, counters, and any entity with a numeric state) display an interactive 24-hour history line graph in their details view, with current, min, and max values, unit of measurement, and hover/pointer inspection.
